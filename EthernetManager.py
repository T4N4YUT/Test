...
class Ethernet_Manager:
    def __init__(self, config_file="ethernet_config.json", default_file="ethernet_default_config.json"):
        ...
        self.intp = Pin(self.config.get_config('int_pin', 4), Pin.IN)
    ...
    async def hardware_reset_lan(self):
        print("🔧 Resetting W5500 (via GPIO14)...")
    ...
    def update_led(self, mode):
        ...
        if mode == 'on':
            ...
            self.led_task = asyncio.create_task(self.led_blink_task(400))
    ...
    async def connect(self, max_retries=3, delay=5):
        ...
        try:
            ...
            print("⚠️ ifconfig failed:", e)

        print("🌐 Trying to connect Ethernet...")

        for attempt in range(max_retries):
            await asyncio.sleep(0.5)
            if self.lan.isconnected():
                print("✅ Ethernet Connected!", self.lan.ifconfig())
                self.update_led('on')
                self.is_connecting = False
        ...
        if self.first_boot:
            print("🔁 First boot failed, rebooting once for cold start recovery...")
            self.first_boot = False
            await asyncio.sleep(2)
            reset()

        print("❌ Ethernet connection failed after retries → rebooting...")
        self.update_led('off')
        self.is_connecting = False
