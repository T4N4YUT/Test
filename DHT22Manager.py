...
class DHT22_Manager:
    def __init__(self, time_manager, ethernet, mqtt_manager, led_manager, config_file='dht22_config.json', default_file='dht22_default_config.json'):
        ...
        self.sensor_locations = {int(k): v for k, v in config.get('SENSOR_LOCATIONS', {str(p): f"Sensor{p}" for p in self.dht22_pins}).items()}
    ...
    async def setup_pins(self):
        ...
        for pin_num in self.dht22_pins:
            try:
                ...
                print(f"Debug: Pin {pin_num} ✔")
            except Exception as e:
                print(f"Error: Pin {pin_num} {e} ✘")
        gc.collect()
    ...
    async def read_sensor(self, sensor):
        try:
            ...
            hum = sensor.humidity()
            if not (self.min_temp_spec <= temp <= self.max_temp_spec): temp = None
            if not (self.min_hum_spec <= hum <= self.max_hum_spec): hum = None
            return temp, hum
        except Exception:
            return None, None
    
    async def collect_data(self, sensor_pin):
        collected_data = {pin: dict(temp_sum=0, hum_sum=0,
    ...
    def calculate_average(self, collected_data):
        ...
        for pin, data in collected_data.items():
            ...
            per_sensor_data[pin] = dict(temp=avg_temp, hum=avg_hum,temp_max=data['temp_max'],
                                        temp_min=data['temp_min'],hum_max=data['hum_max'],hum_min=data['hum_min'])
            if avg_temp is not None:
                total_temp += avg_temp * data['samples_temp']; count_temp += data['samples_temp']
            if avg_hum is not None:
                total_hum += avg_hum * data['samples_hum']; count_hum += data['samples_hum']
        overall = dict(Temperature=round(total_temp / count_temp, 1) if count_temp else None,
    ...
    def calculate_overall_max_min(self, per_sensor_data): 
        ...
        result = dict(Temperature=dict(max=max(temp_val) if temp_val else None, 
    ...
    async def send_or_backup(self, mac, topic, per_sensor, overall):
        ...
        print("🔍 NTP sync:", self.time_manager.ntp_sync)
        print("🌐 Ethernet connected:", self.ethernet.isconnected())
        print("📡 MQTT ready:", self.mqtt_manager.is_mqtt_ready)
        data_row = []
        ...
        if ready:
            timestamp = self.time_manager.get_iso_timestamp()
    ...
    def backup_to_csv(self, records): 
        ...
        print(f"Debug: Backup {len(records)} data ✔")
        gc.collect()
        ...
        if failures: 
            with open(self.backup_csv, 'w') as f: 
                ...
                for line in failures: 
                    f.write(line) 
            print(f"Debug: Backup {len(failures)} data ✔") 
        else: 
            uos.remove(self.backup_csv) 
            print(f"Debug: Deleted" ,self.backup_csv,"✔")
        gc.collect()
    ...
    def send_result(self, per_sensor, overall, result): 
        ...
        for pin, data in per_sensor.items():
            ...
            if data['temp'] is not None and not (self.min_temp_condition - self.per_temp_alarm <= data['temp'] <= self.max_temp_condition + self.per_temp_alarm):
                print(f"Debug: [Alarm!] Pin{pin}({loc}) Temp {data['temp']}°C ")
                is_alarm = True
        ...
        result_str = (f"(Min/Max Temp: {result['Temperature']['min']}°C/{result['Temperature']['max']}°C)   "
                    f"(Min/Max Hum: {result['Humidity']['min']}%/{result['Humidity']['max']}%)")
        print("Debug: [Overall]",overall_str,result_str)
        gc.collect()

    def reset_dht22_config(self):
        try:
            self.config_manager.reset_config(keys=["CON_TEMP_MIN", "CON_TEMP_MAX", "CON_HUM_MIN", "CON_HUM_MAX"])
            print("✅ DHT22 config has been reset to default values.")
        except Exception as e:
            print(f"⚠️ Failed to reset DHT22 config: {e}")
    ...
    async def start_service_dht22(self):
        ...
        while True:
            try:
                ...
                print(f"Error: Start service DHT22 {e} ✘")
                gc.collect()
